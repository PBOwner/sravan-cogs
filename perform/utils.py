...
class PerformUtils:
    ...
    async def send_embed(
        ...
        elif await PerformUtils.has_webhook_perms(ctx):
            try:
                if user:
                    await PerformUtils.print_it(self, ctx, embed, user)
                else:
                    await PerformUtils.print_it(self, ctx, embed)
            except discord.Forbidden:
                if not await PerformUtils.has_embed_perms(ctx):
        ...
        elif not await PerformUtils.has_embed_perms(ctx):
            return await ctx.send(
                "I need the `Embed Links` permission to send embeds in this channel."
            )
        elif user:
    ...
    async def kawaiiembed(
        ...
        try:
            url = await PerformUtils.api_call(f"https://kawaii.red/api/gif/{endpoint}/token={api_key}")
    ...
    async def print_it(
        ...
        hook = await PerformUtils.get_hook(self, ctx)
        try:
            if user:
                await hook.send(
            ...
            await PerformUtils.print_it(self, ctx, embed, retried=True)
